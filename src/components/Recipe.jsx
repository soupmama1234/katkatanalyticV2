...
function RecipeList({ recipes, setRecipes, products, expenses }) {
  ...
  const handleOCR = async (file) => {
    ...
    if (!geminiKey) return alert('กรุณาใส่ Gemini API Key ในหน้าต้นทุนก่อนครับ')
    if (!file) return
    setOcrLoading(true)
    try {
      ...
      const prompt = `อ่านสูตรอาหารจากรูปนี้ return JSON array เท่านั้น ห้ามมีข้อความอื่น
format: [{"item":"ชื่อวัตถุดิบ","quantity":จำนวน,"unit":"หน่วย"}]`
      const resp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${geminiKey}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ inline_data: { mime_type: 'image/jpeg', data: b64 } }, { text: prompt }] }], generationConfig: { temperature: 0.1 } }),
      })
  ...
  const applyOCR = () => {
    const newRows = ocrItems.filter(it => it.item).map(it => ({ ingredient: it.item, quantity: String(it.quantity || ''), unit: it.unit || '' }))
    setIngredients(prev => [...prev.filter(r => r.ingredient), ...newRows])
    setOcrItems([])
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <button onClick={() => { setEditMenu(''); setIngredients([{ ingredient: '', quantity: '', unit: '' }]); setOcrItems([]); setShowModal(true) }}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: '20px 20px 0 0', padding: '20px', width: '100%', maxWidth: 500, maxHeight: '85vh', overflowY: 'auto', border: '1px solid var(--border2)' }}>
            ...
            <label style={{
              ...
              borderRadius: 12, padding: '12px', color: '#4D96FF', fontWeight: 700, fontSize: 13,
              cursor: 'pointer', marginBottom: 12, fontFamily: 'inherit',
            }}>
              {ocrLoading ? '⏳ กำลังอ่านสูตร...' : '📸 สแกนสูตรจากรูปภาพ'}
              {!ocrLoading && <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handleOCR(e.target.files[0])} />}
            </label>
            {ocrItems.length > 0 && (
              <div style={{ background: 'rgba(77,150,255,0.08)', border: '1px solid #4D96FF33', borderRadius: 10, padding: 12, marginBottom: 12 }}>
                <div style={{ fontSize: 12, color: '#4D96FF', fontWeight: 700, marginBottom: 8 }}>พบ {ocrItems.length} รายการ</div>
                {ocrItems.map((it, i) => (
                  <div key={i} style={{ fontSize: 12, color: 'var(--dim)', marginBottom: 2 }}>{it.item} — {it.quantity} {it.unit}</div>
                ))}
                <button onClick={applyOCR} style={{ marginTop: 8, width: '100%', background: '#4D96FF', color: '#000', border: 'none', borderRadius: 8, padding: '10px', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
                  ➕ เติมลงในสูตร
                </button>
              </div>
            )}
